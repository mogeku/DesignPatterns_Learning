...
// Service+Domain	 业务逻辑处理层 领域模型(充血模型)
class VirtualWalletDomain	
{
public:
	VirtualWalletDomain(const VirtualWalletEntity& virtual_wallet_entity)
	{
	}
...
public:
	...
	{
		int ret = 0;
		VirtualWalletEntity wallet_entity = virtual_wallet_repo.GetWalletById(id);
		VirtualWalletDomain wallet_domain(wallet_entity);
		...
		wallet_domain.Debit(amount);
		if (0 != virtual_wallet_repo.UpdataBalance(id, wallet_domain.Balance()))
		{
